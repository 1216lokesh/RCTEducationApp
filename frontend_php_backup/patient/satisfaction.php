...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array(-1, $answers, true)) {
        ...
        if ($check) {
            $db->query("
                UPDATE satisfaction_scores SET score = {$score} 
                WHERE patient_id = {$userId} AND procedure_id = {$procedureId} AND timepoint = '{$timepoint}'
            ");
        } else {
            $db->query("
                INSERT INTO satisfaction_scores (patient_id, procedure_id, timepoint, score)
...
$opts = [
    1 => __('satisfaction_opt1'), // Strongly Disagree
    2 => __('satisfaction_opt2'), // Disagree
    3 => __('satisfaction_opt3'), // Neutral
    4 => __('satisfaction_opt4'), // Agree
    5 => __('satisfaction_opt5')  // Strongly Agree
];
        ...
        <div class="col-md-8 col-lg-6">
            ...
            <div class="card shadow-sm border-0 rounded-3 p-4">
                <h3 class="fw-bold mb-2 text-primary-custom" style="color: #1565C0;">
                    <?php echo __('satisfaction_title'); ?>
                </h3>
                <p class="text-muted mb-4"><?php echo __('satisfaction_subtitle'); ?></p>
